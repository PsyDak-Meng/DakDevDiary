...
### 2.1.2 Context
**Context Encoder:** ViT encodes context image to embedding ![S_x](https://latex.codecogs.com/svg.image?S_x)
**Mask Token**: an [1,D] randomly initialized shared learnable vector ![M](https://latex.codecogs.com/svg.image?M), values are used where it is a masked pactc (colored pacthes in the figure).
**Positional embedding**: [1,D] sinusoidal embedding ![PE](https://latex.codecogs.com/svg.image?PE)
**Predictor**: standard ViT, inputs masked token, context embedding and positional embedding for
<div style="text-align: center;">
![equation](https://latex.codecogs.com/svg.image?g_%7B%5Cphi%7D%28M%20%2B%20PE%20%2B%20S_x%29%20%3D%20%5Chat%7By%7D)
</div>

### 2.1.3 Target
**Target Encoder**: input original image and masked bbox to get corresponding embedding patches as ![y](https://latex.codecogs.com/svg.image?y). 

### 2.1.4 Loss & Training
The avg. ![L_2](https://latex.codecogs.com/svg.image?L_2) distance between predicted ![\hat{y}](https://latex.codecogs.com/svg.image?%5Chat%7By%7D) and target ![y](https://latex.codecogs.com/svg.image?y).
![equation](https://latex.codecogs.com/svg.image?%5Cmathcal%7BL%7D%20%3D%20%5Cfrac%7B1%7D%7B%7C%5Cmathcal%7BM%7D%7C%7D%20%5Csum_%7B%28i%2Cj%29%20%5Cin%20%5Cmathcal%7BM%7D%7D%20%5Cleft%7C%20g_%7B%5Cphi%7D%28f_%7B%5Ctheta%7D%28x_v%29%2C%5C%20PE_%7Bi%2Cj%7D%29%20-%20f_%7B%5Cxi%7D%28x%29_%7Bi%2Cj%7D%20%5Cright%7C%5E2)

Where:
<div style="text-align: center;">
![f_{\theta}](https://latex.codecogs.com/svg.image?f_%7B%5Ctheta%7D) — context encoder (trained), processes visible patches ![x_v](https://latex.codecogs.com/svg.image?x_v)

![f_{\xi}](https://latex.codecogs.com/svg.image?f_%7B%5Cxi%7D) — target encoder (EMA), processes full image ![x](https://latex.codecogs.com/svg.image?x)

![g_{\phi}](https://latex.codecogs.com/svg.image?g_%7B%5Cphi%7D) — predictor (trained), takes context representations + positional embedding of masked position

![\mathcal{M}](https://latex.codecogs.com/svg.image?%5Cmathcal%7BM%7D) — set of masked patch positions

![PE_{i,j}](https://latex.codecogs.com/svg.image?PE_%7Bi%2Cj%7D) — positional embedding at masked position ![(i,j)](https://latex.codecogs.com/svg.image?%28i%2Cj%29)
</div>
<hr>
Target encoder's EMA (parameters update not in loss by gradient descent, but direct update after each step):
<div style="text-align: center;">
![equation](https://latex.codecogs.com/svg.image?%5Cxi_t%20%3D%20%5Calpha%20%5Cxi_%7Bt-1%7D%20%2B%20%281-%5Calpha%29%5Ctheta_%7Bt-1%7D)
![equation](https://latex.codecogs.com/svg.image?%3D%20%281-%5Calpha%29%5Csum_%7Bk%3D0%7D%5E%7Bt%7D%20%5Calpha%5Ek%20%5Ctheta_%7Bt-k%7D)
</div>

The weight of a past context encoder snapshot ![\theta_{t-k}](https://latex.codecogs.com/svg.image?%5Ctheta_%7Bt-k%7D) decays as ![\alpha^k](https://latex.codecogs.com/svg.image?%5Calpha%5Ek) — exponentially in how many steps ago it was. That's where the name comes from.

So with ![\alpha=0.996](https://latex.codecogs.com/svg.image?%5Calpha%3D0.996):

1 step ago: weight ![= 0.996^1 = 0.996](https://latex.codecogs.com/svg.image?%3D%200.996%5E1%20%3D%200.996)
100 steps ago: weight ![= 0.996^{100} \approx 0.67](https://latex.codecogs.com/svg.image?%3D%200.996%5E%7B100%7D%20%5Capprox%200.67)
1000 steps ago: weight ![= 0.996^{1000} \approx 0.02](https://latex.codecogs.com/svg.image?%3D%200.996%5E%7B1000%7D%20%5Capprox%200.02)
...
<div style="text-align: center;">
![equation](https://latex.codecogs.com/svg.image?%5Cmax%5C%20I%28%5Chat%7Bz%7D%3B%5C%20f_%7B%5Cxi%7D%28x%29%29)
</div>
Rather than maximizing ![I(\hat{z};\ x)](https://latex.codecogs.com/svg.image?I%28%5Chat%7Bz%7D%3B%5C%20x%29) directly. Since ![f_{\xi}(x)](https://latex.codecogs.com/svg.image?f_%7B%5Cxi%7D%28x%29) already has low-level entropy compressed away, the predictor only needs to capture what's semantically predictable — the mutual information that survives the encoder bottleneck.
